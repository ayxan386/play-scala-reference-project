...
import error.JsonBodyNotProvided
...
class CommentController @Inject()(
  ...
  def addComment() = Action.async { implicit request =>
    val author =
      request.headers.get("Author").getOrElse(throw AuthorNotProvided())
    request.body.asJson match {
      case Some(jsonBody) =>
        val parsed = jsonBody.as[CommentInput]
        val dto = CommentDTO(body = parsed.body, None)
        commentService
          .addComment(dto, parsed.postId, author)
          .map(dto => Ok(Json.toJson(dto)))
      case None => throw JsonBodyNotProvided()
    }
  }
